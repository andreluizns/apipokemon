...
- **Filtro por geração**: usa `/generation/{nome}`, que retorna a lista completa de espécies daquela geração. Tipo, geração e busca por nome são resolvidos e intersectados em `resolvePokemonCandidates` **antes** da paginação — evita o bug de, por exemplo, filtrar por "Geração IX" e o "Carregar mais" parecer não trazer nada, já que sem essa interseção prévia cada página de 20 candidatos (ordenados a partir do Pokémon nº1) precisaria ser individualmente verificada contra a geração, e as gerações mais recentes só apareceriam dezenas de cliques depois.
- **Filtros de altura/peso**: esses campos só existem no endpoint de detalhe (`/pokemon/{id}`), não no de listagem nem no de geração/tipo. Por isso são aplicados no cliente sobre os detalhes já carregados (`filterByHeightAndWeight`), depois que tipo/geração/nome já reduziram a lista de candidatos — combinar um filtro de altura/peso muito restritivo com "carregar mais" ainda pode fazer uma página aparente ter menos de 20 itens, já que esse filtro é local ao lote buscado, não ao total do dataset. Tradeoff assumido conscientemente por não haver esse filtro no lado do servidor.
